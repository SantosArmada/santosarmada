var Framer = {
    ...
    drawTick: function (x1, y1, x2, y2) {
        ...
        gradient.addColorStop(0, '#D91729');
        gradient.addColorStop(0.6, '#D91729');
        gradient.addColorStop(1, '#F2EEE3');
        this.context.beginPath();
    ...
    drawEdging: function () {
        this.context.save();
        this.context.beginPath();
        this.context.strokeStyle = 'rgba(217, 23, 41, 0.5)';
        this.context.lineWidth = 1;
...
var Tracker = {
    ...
    drawArc: function () {
        this.context.save();
        this.context.strokeStyle = 'rgba(217, 23, 41, 0.8)';
        this.context.beginPath();
...
var Scene = {
    ...
    canvasConfigure: function () {
        ...
        this.context.strokeStyle = '#D91729';
        this.calculateSize();
    },
...
var Controls = {
    ...
    drawPic: function () {
        this.context.save();
        this.context.beginPath();
        this.context.fillStyle = 'rgba(217, 23, 41, 0.85)';
        this.context.lineWidth = 1;
...
/* ---------------------------------------------------------
   PLAYER — rebuilt on top of a real <audio> element instead
   of a manual fetch + decodeAudioData. This is what makes the
   scrubber actually work, and it's the same hook point you'll
   use later: swap audio.src per chapter once each one is recorded.
--------------------------------------------------------- */
var Player = {
...
/* ---------------------------------------------------------
   VOLUME DIAL — vanilla-JS port of the provided React/GSAP
   rotary dial. Drag the knob (or use arrow keys once it has
   focus) to set volume; wired straight into Player.setVolume.
--------------------------------------------------------- */
var VolumeDial = {
...
/* ---------------------------------------------------------
   POWER BUTTON — just the on/off toggle from the provided
   dial-panel SVG. Turns the player (and the volume dial) off:
   pauses playback and dims/disables both until switched back on.
--------------------------------------------------------- */
var PlayerPower = {
...
/* ---------------------------------------------------------
   BLOOD SCROLL EFFECT
--------------------------------------------------------- */
function initBloodScroll() {
    var fill = document.querySelector('.blood-fill');
    if (!fill) { return; }
    function update() {
